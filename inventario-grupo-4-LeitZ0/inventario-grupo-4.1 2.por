programa
{
	...
	funcao inteiro menuEscolherOpcao()
	{
		...
		faca
		{
			...
			escreva(
				...
				"[3] Imprimir produtos\n",
				"[4] Imprimir em Ordem Alfabética\n",
				"[5] Sair\n",
				LINHA_SIMPLES,
				"--> "
				)
	...
	funcao Ordem(){
        ...
        ordenarVetorCadeia(vProdutos, vProdutosC)
        para (inteiro i=0; i<T; i++) {
			escreva(vProdutos[i] +"\n")
        }
	}
	...
	funcao ordenarVetorCadeia(cadeia vProdutos[], caracter vProdutosC[]) {
        para (inteiro i=0; i<T; i++) {            
	...
	funcao obterPrimeiroCaracterVetor(cadeia v[], caracter vProdutosC[]) {
        para (inteiro i=0; i<T; i++) {
            vProdutosC[i] = txt.obter_caracter(txt.caixa_alta(vProdutos[i]), 0)
        }
	}
 ...
 * 
 * @POSICAO-CURSOR = 1880; 
 * @DOBRAMENTO-CODIGO = [17, 25, 42, 114, 122, 126, 145];
 * @PONTOS-DE-PARADA = ;
